...
ADMIN_EMAIL=...
ADMIN_PASSWORD=... # optional emergency fallback only
SESSION_SECRET=...
GOOGLE_CLIENT_ID=...
...
- Admin OAuth should be restricted to the exact Google account in `ADMIN_EMAIL`. For your setup, point that at the shared `pawprintstogoco` Google account and register the callback URL exactly as `GOOGLE_OAUTH_REDIRECT_URI`.
