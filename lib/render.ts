...
type PosterCompositionInput = {
  ...
  cleanedPortrait: Buffer;
  portraitLeft: number;
  portraitTop: number;
  titleSafeBand: Buffer;
...
const POSTER_QA_FOREGROUND_DISTANCE_THRESHOLD = 45;
...
async function buildPosterPng(
  ...
  const secondLineOverlay = title.secondLine
    ...
    : null;
  const compositionInput: PosterCompositionInput = {
    ...
    titleOverlays: [
      {
        buffer: firstLineOverlay.buffer,
      ...
      ...(secondLineOverlay
        ? [
            {
              buffer: secondLineOverlay.buffer,
...
async function composePosterPng(
  ...
  return sharp({
    create: {
      ...
      height: FINAL_HEIGHT,
      channels: 4,
      background: input.backgroundFill
    }
  })
    .composite([
...
export async function analyzePosterComposition(
  poster: Buffer,
  backgroundFill: string
): Promise<PosterCompositionQaReport> {
  const background = parseHexColor(backgroundFill);
  ...
  const titleBounds = createPixelAccumulator();
  const petBounds = createPixelAccumulator();
  let petPixelCount = 0;
  let bottomContactPixels = 0;
  ...
  for (let y = 0; y < info.height; y += 1) {
    for (let x = 0; x < info.width; x += 1) {
      ...
      if (y < TITLE_SAFE_HEIGHT) {
        ...
        if (y >= info.height - 2) {
          bottomContactPixels += 1;
        }
      }
    }
  }

  const titleCenterDeltaPx = titleBounds.hasPixels()
    ? Number((titleBounds.boundsCenterX() - centerX).toFixed(2))
    : null;
  const petBoundsCenterDeltaPx = petBounds.hasPixels()
    ? Number((petBounds.boundsCenterX() - centerX).toFixed(2))
    : null;
  ...
  const bottomContact = bottomContactPixels >= POSTER_QA_BOTTOM_CONTACT_MIN_PIXELS;
  const warnings: string[] = [];

  if (
    titleCenterDeltaPx !== null &&
    Math.abs(titleCenterDeltaPx) > POSTER_QA_CENTER_TOLERANCE_PX
  ) {
    warnings.push("title_off_center");
  }

  if (
    petVisualCenterDeltaPx !== null &&
    Math.abs(petVisualCenterDeltaPx) > POSTER_QA_CENTER_TOLERANCE_PX
  ) {
    warnings.push("pet_visual_center_off");
  }
...
export function calculatePosterCompositionCorrections(
...
function logPosterCompositionQa(
  ...
  console.log(
    `[render] poster QA ${label}: ${JSON.stringify({
      ...report,
      corrections
    })}`
  );
...
async function generateAiPortrait(source: Buffer, petName: string) {
  ...
  form.append(
    ...
    [
      ...
      "",
      "STYLE:",
      "Render as a premium stylized illustrated pet portrait for a modern Etsy-style print.",
      "The result should feel elegant, editorial, refined, and clearly illustrated rather than photographic.",
      "Use clean graphic shapes, controlled simplification, and painterly fur strokes while keeping natural anatomy, believable facial proportions, and true expression.",
      "Aim for the polished custom poster style of high-end digital pet art: simplified planes, crisp feature readability, visible brush-like fur grouping, and tasteful stylization.",
      "Shading should be soft and restrained, with deliberate illustrated tonal blocks rather than photographic micro-texture or glossy cartoon airbrushing.",
      ...
      "This should feel like a polished custom portrait print, not a photo retouch, children's-book illustration, mascot logo, or exaggerated cartoon.",
      "Retain layered illustrated fur detail and clear feature definition in the eyes, nose, muzzle, forehead, and fur transitions.",
      "Do not flatten, over-smooth, or oversimplify important fur structure, but avoid hyperreal skin, wet-nose photography, and lifelike camera-rendered texture.",
      "",
      "DETAIL RENDERING:",
      "Increase illustrated detail in the following areas:",
      ...
      "Preserve convincing depth in the face and muzzle using stylized value shapes so the portrait does not become flat, generic, or icon-like.",
      "",
      ...
      "- no photo retouching look",
      "- no camera-like texture",
      "- no hyperrealistic fur",
      ...
      "- elegant stylized illustrated print styling",
